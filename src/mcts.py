...
# MMLU letter parsing — more robust to markdown, punctuation, parentheses, etc.
...
def extract_final_letter(text: str):
    """
...
def extract_pred_number(text: str):
    m = re.findall(r"answer\s*:\s*([-\$]?\s*\d[\d,]*(?:\.\d+)?(?:\s+\w+)?)", text, flags=re.I)
